...
export default function Projects() {
  ...
  return (
    ...
    >
      ...
      <motion.div style={{ x }} className="flex gap-8 px-6 md:px-12 lg:px-24 pb-16 w-max">
        {projects.map((project) => (
          <motion.div
            ...
            className={cn(
              "relative rounded-2xl border bg-[#0D0D0D] backdrop-blur-sm flex-shrink-0 group cursor-pointer transition-all duration-500",
              project.accent,
              hoveredId === project.id ? "border-white/30" : "",
              project.special ? "w-[500px] md:w-[600px]" : "w-[380px] md:w-[450px]"
            )}
          >
            ...
            <div className="p-6 md:p-8">
              <div className="flex items-start justify-between mb-6">
                ...
                {project.special && (
                  <span className="text-[10px] terminal-text text-white/60 border border-white/10 rounded-full px-3 py-1">
                    ★ Featured
                  </span>
                )}
              </div>
